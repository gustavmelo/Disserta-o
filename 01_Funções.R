...
clusterizar <- function(dorig, m, h){
  ...
  for(i in 1:20){  #Alterar se preferir aumentar ou reduzir num max de clusters
...
matriz_inter<-function(i){
  ...
  cl1 <- data.frame(clusters[24*i]) #vetor com todas as últimas horas do mês i (todos os dias)
  cl2 <- data.frame(clusters[(24*i)+1]) #vetor com todas as primeiras horas do mês i (todos os dias)
  
  num_anos<-min(nrow(cl1)/diasmes[i],nrow(cl2)/diasmes[i+1]) #nem sempre o primeiro ou último ano serão completos, considerar o número de anos mínimo
  
  for (j in 1:num_anos) {
    estadolinha <- cl1[[1]][[diasmes[i]*j]] #ultimo horário do mês i
    estadocoluna <- cl2[[1]][[(j-1)*diasmes[i+1]+1]] #primeiro horário do mês i+1
    Pind_matriz[estadolinha,estadocoluna] = Pind_matriz[estadolinha,estadocoluna] + 1
  }
  for(z in 1:nrow(Pind_matriz)){
    soma = sum(Pind_matriz[z,])
